...
// 🔥 YOUR OLD API KEY (WORKING KEY)
const OPENWEATHER_KEY = "5e79e7210cb543fea4a97220f8dbdd08";
...
export default function Data() {
  ...
  /* ============================================================
     WEATHER FETCH (REAL OPENWEATHER)
  ============================================================ */
  const fetchWeatherRecords = async () => {
    try {
      // Using OpenWeather API for Tanauan, Batangas
      const url = `https://api.openweathermap.org/data/2.5/weather?q=Tanauan,PH&appid=${OPENWEATHER_KEY}&units=metric`;
      ...
      setWeatherRecords([record]); // Latest only
    } catch (err) {
      console.error("WEATHER ERROR:", err);
      toast.error("Failed to load live weather data");
    }
  };
                ...
                <table className="w-full text-sm border">
                  ...
                  <tbody>
                    <tr className="border-b">
                      <td className="p-2">
                        {new Date(
                          weatherRecords[0].date
                        ).toLocaleDateString()}
                      </td>
                      <td className="p-2">
                        {weatherRecords[0].temperature.toFixed(1)}
                      </td>
                      <td className="p-2">
                        {weatherRecords[0].humidity.toFixed(0)}%
                      </td>
